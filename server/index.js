...
const fail = (req, res, status, err) => {
  log.warn(
    { error: err.stack, origin: "express", path: req.path },
...
const go = (config) => {
  ...
  const settlementInclude = [
    { model: Order, as: "order", include: [{ model: Asset, as: "asset" }] },
  ];
  const orderInclude = [
  ...
  router.get("/settlements/:transactionHashIndex", (req, res) => {
    return Settlement.findOne({
      where: { transactionHashIndex: req.params.transactionHashIndex },
  ...
  router.get("/orders", (req, res) => {
    ...
    if (req.query.schema) {
      include = [
        { model: Asset, as: "asset", where: { schema: req.query.schema } },
        { model: Settlement, as: "settlement" },
      ];
    }
  ...
  router.post("/orders/post", async (req, res) => {
    const order = req.body;

    let { schema, formatted, asset } = await validateOrder(
      web3,
      protocolInstance,
      order,
      config
    ...
    const hash = WyvernProtocol.getAssetHashHex(
      schema.hash(asset),
      schema.name
    );
    ...
    return Order.create(encodeOrder(order, asset)).then(() => {
      res.json({ result: true, error: null });
    });
  ...
  router.post("/orders/validate", (req, res) => {
    const order = req.body;
    return validateOrder(web3, protocolInstance, order, config)
      .then(() => {
        res.json({ result: true, error: null });
      })
  ...
  sequelize
    ...
    .then(async () => {
      ...
      if (process.env.WYVERN_SYNC_ORDERBOOK) {
        ...
        syncLogs(web3, protocolInstance, {
          sequelize,
          Sequelize,
          Op,
          Synced,
        ...
        schemas[network]
          .filter((s) => s.events.transfer.length > 0)
          .map((s) =>
            syncAssets(s, web3, protocolInstance, {
              sequelize,
              Sequelize,
              Synced,
              Asset,
